...
# 确保输入包含年月日时信息
if len(input_list) >= 4:
    year, month, day, hour = input_list[:4]

    # 从用户输入中获取性别信息
    gender_option = "-n"
    gender = "female" if gender_option in user_input else "male"

    # 用户输入 registration 和 feedback 内容
    registration_text = input("请输入籍贯：")
    feedback_text = input("请输入目前状况：")

    # 构建命令
    command = ["python3", "./bazi.py"] + input_list

    # 执行命令并获取输出内容
    output_content = subprocess.run(command, stdout=subprocess.PIPE, text=True).stdout

    # 使用正则表达式匹配 "四柱: " 后面的内容
    match = re.search(r'四柱:\s*(.*)', output_content)

    # 如果匹配成功，获取匹配的内容并设置为 ba
    ba = match.group(1).strip() if match else None
    ...
    # 执行 SQL 插入语句
    insert_query = "INSERT INTO demo (year, month, day, hour, gender, content, registration, feedback, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
    cursor.execute(insert_query, (year, month, day, hour, gender, output_content, registration_text, feedback_text, current_time))
    ...
    print("输入不足，请提供年月日时信息。")
    # 可以根据需要进行其他处理或要求用户重新输入
...
print("-"*120)
print("墓库：", str(kus).replace("'",""), "解读:v信maru8022", end=' ')
for item in zhus:
    print(''.join(item), end=' ')
print()
...
if options.b:
    ...
    birthday = date(day.getSolarYear(), day.getSolarMonth(), day.getSolarDay())
    count = 0
...
print("\n\n五行缺{}的建议参见 公众号:金水易".format(short))    
